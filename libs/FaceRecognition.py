...
class FaceRecognizer:
    ...
    def fit(self):
        ...
        all_images = np.zeros((self.total_images, self.img_shape[0], self.img_shape[1]), dtype='float64')
        i = 0
        ...
        A = np.resize(all_images, (self.total_images, self.img_shape[0] * self.img_shape[1]))
    ...
    def detect_face(self, source_path: str, source: np.ndarray = None):
        # testing image
        # test_img = cv2.imread('test.jpeg', cv2.IMREAD_GRAYSCALE)
        # test_img = cv2.imread('test2.png', cv2.IMREAD_GRAYSCALE)
        ...
        omega = self.eigenfaces[:q].dot(mean_subtracted_test_img)
        print(omega.shape)
        ...
        projected_new_img_vector = self.eigenfaces[:q].T @ omega
    ...
    def recognize_face(self, source_path: str, source: np.ndarray = None):
        # testing image
        # test_img = cv2.imread('test.jpeg', cv2.IMREAD_GRAYSCALE)
        # test_img = cv2.imread('test2.png', cv2.IMREAD_GRAYSCALE)
        test_img = cv2.imread(source_path, cv2.IMREAD_GRAYSCALE)
        ...
        for k in range(self.total_images):
            ...
            omega_k = self.eigenfaces[:q].dot(self.A_tilde[k])
            diff = omega - omega_k
        ...
        if smallest_value < alpha_2:
            print(smallest_value, self.names[index])
        else:
...
if __name__ == "__main__":
    ...
    recognizer.recognize_face(source_path=image_path)