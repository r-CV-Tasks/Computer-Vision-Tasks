...
def apply_sift(source: np.ndarray, source2: np.ndarray) -> np.ndarray:
    """
...
def isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
    """
        Return True if the center element of the 3x3x3 input array is strictly greater than or
        less than all its neighbors, False otherwise
    """
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    kp, dc = computeKeypointsAndDescriptors(img)
    t2 = time.time()
